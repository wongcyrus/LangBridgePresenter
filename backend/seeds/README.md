...
2.  **Google Cloud Credentials**: Your environment must be authenticated with Google Cloud by running `gcloud auth application-default login`. You will also need permissions for Firestore, Cloud Storage, and Text-to-Speech.
3.  **Infrastructure Config**: The script relies on `backend/cdktf_outputs.json` (generated by `deploy.sh` or `cdktn deploy`) to know the project IDs and bucket names.
