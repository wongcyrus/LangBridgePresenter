...
def grant_access(uid: Optional[str], email: Optional[str], note: str):
    db = _get_db()
    ...
    db.collection("voice_chat_users").document(doc_id).set(payload, merge=True)
    print(f"Granted voice chat access: {doc_id}")


def revoke_access(uid: Optional[str], email: Optional[str]):
    db = _get_db()
    doc_id = _doc_id(uid, email)
    ref = db.collection("voice_chat_users").document(doc_id)
    snap = ref.get()
    if not snap.exists:
        print(f"Voice chat user not found: {doc_id}")
        return
    ref.set({"active": False, "updated_at": firestore.SERVER_TIMESTAMP}, merge=True)
    print(f"Revoked voice chat access: {doc_id}")


def delete_access(uid: Optional[str], email: Optional[str]):
    db = _get_db()
    doc_id = _doc_id(uid, email)
    db.collection("voice_chat_users").document(doc_id).delete()
    print(f"Deleted voice chat access record: {doc_id}")


def list_access(active_only: bool):
    db = _get_db()
    query = db.collection("voice_chat_users")
    if active_only:
        query = query.where("active", "==", True)

    docs = list(query.stream())
    if not docs:
        print("No voice chat users found.")
        return
...
def main():
    parser = argparse.ArgumentParser(description="Manage voice chat access users")
    ...
    parser_grant = subparsers.add_parser("grant", help="Grant voice chat access")
    ...
    parser_revoke = subparsers.add_parser("revoke", help="Revoke voice chat access")
    parser_revoke.add_argument("--uid", help="Firebase UID")
    parser_revoke.add_argument("--email", help="User email")

    parser_delete = subparsers.add_parser("delete", help="Delete voice chat access record")
    parser_delete.add_argument("--uid", help="Firebase UID")
    parser_delete.add_argument("--email", help="User email")

    parser_list = subparsers.add_parser("list", help="List voice chat access users")
    parser_list.add_argument("--active-only", action="store_true", help="Show only active users")
    ...
    if args.command == "grant":
        grant_access(args.uid, args.email, args.note)
    elif args.command == "revoke":
        revoke_access(args.uid, args.email)
    elif args.command == "delete":
        delete_access(args.uid, args.email)
    elif args.command == "list":
        list_access(args.active_only)
