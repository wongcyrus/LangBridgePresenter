...
def _has_text_access(decoded_token: dict, db: firestore.Client) -> bool:
...
def text_chat_access(request: Request):
    ...
    try:
        ...
        logger.warning("text_chat_access auth failed: %s", e)
        return (json.dumps({"error": "Unauthorized"}), 401, headers)

    db = firestore.Client(database="langbridge")
    granted = _has_text_access(decoded, db)
    payload = {
        "granted": granted,
        "uid": decoded.get("uid"),
    ...
    if not granted:
        payload["reason"] = "Text chat access requires admin grant"
